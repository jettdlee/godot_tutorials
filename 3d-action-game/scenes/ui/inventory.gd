...
const border_colours = [Color.LIME_GREEN, Color.FIREBRICK, Color.GOLDENROD]
...
func _process(delta: float) -> void:
	var pan_direction = Input.get_axis('pan_left', 'pan_right')
	player_preview.rotate_y(pan_direction * delta * pan_speed)
...
func _input(_event: InputEvent) -> void:
	if Input.is_action_just_pressed('menu'):
		Global.pause(false)
	var menu_direction = int(Input.get_axis('switch_shield', 'switch_weapon'))
	if menu_direction:
		tab_container.current_tab = posmod(tab_container.current_tab + menu_direction, tab_container.get_child_count())
		focus()
		$HBoxContainer/ItemPanelContainer/Control/Label.text = tab_container.get_child(tab_container.current_tab).name
		var color = border_colours[tab_container.current_tab]
